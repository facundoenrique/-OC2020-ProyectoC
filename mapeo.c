...
/**
    Funciones para encontrar un numero primo cuando se agranda la tabla hash
**/
...
extern void crear_mapeo(tMapeo * m, int ci, int (*fHash)(void *), int (*fComparacion)(void *, void *))
{
    *m=(tMapeo)malloc(sizeof(struct mapeo));
    ...
    (*m)->tabla_hash=(tLista*)malloc(longitud_t*sizeof(tLista));
...
extern tValor m_insertar(tMapeo m, tClave c, tValor v)
{
    ...
    //De ser asi agrando el arreglo y "corro" las listas del arreglo anterior al nuevo
    //O agrando el arreglo, inicializo las nuevas listas y copio las entradas en las posiciones correspondientes
    //de la nueva lista?

    if (m->cantidad_elementos+1>=(75*(m->longitud_tabla)/100))
    {
        int longitud_nueva=proximo_primo(m->longitud_tabla*2);
        //int longitud_nueva=m->longitud_tabla*2;
        tLista *nuevo_arreglo=(tLista*)malloc(longitud_nueva * sizeof(tLista));
        if(nuevo_arreglo==NULL)
            exit(MAP_ERROR_MEMORIA);
        for (int i=0;i!=longitud_nueva;i++)
        {
        //tLista lista_de_entradas_nueva;
       // crear_lista(&lista_de_entradas_nueva);
        //*(nuevo_arreglo+i)=lista_de_entradas_nueva;
        crear_lista(&*(nuevo_arreglo+i));
        //*(nuevo_arreglo+i)=lista_de_entradas;
        }
        ...
        for (int i=0;i!=longitud_anterior;i++)
        {
            ...
            /*
            tEntrada entrada_anterior=(tEntrada)l_recuperar((*(m->tabla_hash+i)),l_primera(*(m->tabla_hash+i)));
            tClave clave_anterior=entrada_anterior->clave;
            int clave_nueva=m->hash_code(clave_anterior);
            l_destruir(*(nuevo_arreglo+clave_nueva),fEliminarEntradas);
            *(nuevo_arreglo+clave_nueva)=(*(m->tabla_hash+i));
            //l_destruir(*(m->tabla_hash+i),fEliminarEntradas);
            */

            //ESTO ES LO NUEVO, COPIANDO CADA ENTRADA A LA LISTA NUEVA CREADA:
...
//--------------------------------------------------------------------------------------------------//
    ...
    else
    {
        ...
        /*
        tEntrada ultima_entrada=(tEntrada)l_recuperar(*(m->tabla_hash+clave),l_ultima(*(m->tabla_hash+clave)));
        if (m->comparador(ultima_entrada->clave,c)==0)
        {
            esta_la_clave=1;
            entrada_que_se_esta_viendo=ultima_entrada;
        }
        */
...
extern void m_destruir(tMapeo * m, void (*fEliminarC)(void *), void (*fEliminarV)(void *))
{
    funcion_eliminar_claves=*fEliminarC;
    funcion_eliminar_valores=*fEliminarV;
    for (int i=0;i!=(*m)->cantidad_elementos;i++)
    {
